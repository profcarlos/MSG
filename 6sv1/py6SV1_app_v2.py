...
class corr_atm():
	...
	def __init__(self):
		...
		self.START = [[550, 651, 25, 10], [1000, 1126, 25, 10], [0, 701, 100, 10], [0, 361, 60, 1], [0, 91, 15, 10],[0, 361, 60, 1000], [0, 901, 100, 1000]]		
		...
		for vza in (self.drange(self.START[0][:3])):
			...
			print('.Reading data file: %s'%(path))
			#print(vza)
			if(vza == 550):
		...
		for vza in (self.drange(self.START[0][:3])):
			path = os.getcwd() + '\\LUT_6S_2\\LUT_6S_VZA_' + str(vza) + '.csv'
			print('.Reading data file: %s'%(path))
			DATA = numpy.append(DATA, numpy.loadtxt(path, dtype='int32', delimiter = ' ', skiprows = 1), axis = 0)
			#print('DATA [%d][%d]' %(len(DATA), len(DATA[0])))
...
class main():
	...
	if ("-ds" in argDict and "-de" in argDict and "-fo" in argDict and "-fi" in argDict):
		...
		exit(usage)
	six = corr_atm()
	HOURS = [1300, 1315, 1330, 1345, 1400, 1415, 1430, 1445, 1500, 1515, 1530, 1545, 1600, 1615, 1630, 1645, 1700]
	ds = gdal.Open(fileMask123)
	y_123 = ds.RasterYSize
	x_123 = ds.RasterXSize
		...
		#for hour in random.sample(HOURS, 2):
			...
			for x in range(x_123):
				for y in range(y_123):
					...
					[red, nir] = six.calc_corr(ADJ, DATA[4][y][x], DATA[5][y][x])
