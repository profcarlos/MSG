...
START = [[550, 650, 25, 10], [1075, 1100, 25, 10], [0, 601, 100, 10], [0, 360, 60, 10], [0, 76, 15, 10],[0, 360, 60, 1000], [000, 801, 100, 1000]]
...
class main():
	...
	lut_file = os.getcwd() + '\\LUT_6S_3bands\\LUT_6S_VZA_' + str(VZA) + '_2.csv'
	num = 0
				...
				for WV in drange(START[4][:3]):
					for OZ in drange(START[5][:3]):
						for AOT in drange(START[6][:3]):
							...
							RED, NIR, SWIR = sixsv1(VZA/START[0][3], VAZ/START[1][3], SZA/START[2][3], SAZ/START[3][3], WV/START[4][3], OZ/START[5][3], AOT/START[6][3])
							...
							SWIR = [int(float(SWIR[0])*100000), int(float(SWIR[1])*100000), int(float(SWIR[2])*100000)]
							newrow = [VZA, VAZ, SZA, SAZ, WV, OZ, AOT, RED[0], RED[1], RED[2], NIR[0], NIR[1], NIR[2], SWIR[0], SWIR[1], SWIR[2]]
							print('. 6SV1 at : %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d'%(VZA, VAZ, SZA, SAZ, WV, OZ, AOT, RED[0], RED[1], RED[2], NIR[0], NIR[1], NIR[2], SWIR[0], SWIR[1], SWIR[2]))	
							if(len(DATA) == num):
					...
					if(not(len(DATA) > num)):
						print('. Save data')
						numpy.savetxt(lut_file, DATA[:num], fmt='%d', comments = '', header = 'VZA VAZ SZA SAZ WV OZ AOT XA_RED XB_RED XC_RED XA_NIR XB_NIR XC_NIR XA_SWIR XB_SWIR XC_SWIR ', delimiter = ' ', newline='\n')
