...
class main():
	...
	#output_filename = 'T:\\OUTROS\\TerraClass\\TClass_maskVectorGoias_go_go_perc_class_num_otto.tif'
	...
	for band in range(1,bands+1,1):
		...
		DATA = readFileBand(datafile, band)
		for basin in range(1,n_basin+1,1):
			sample = numpy.sum(DATA[OTTO == basin])
			print('...process %d basin has %.2f'%(basin, sample))
		...
		saveRASTERfile(output_filename.replace('num', str(doy)), ottofile, [MEAN_DATA, STD_DATA])
		print('...saving file %s'%(output_filename.replace('num', str(doy))))
		sys.exit()
		doy = doy + 1
		#OUT_DATA[band-1] = MEAN_DATA 
		OUT_DATA = MEAN_DATA 
		MEAN_DATA = numpy.zeros((y_band, x_band))
		STD_DATA = numpy.zeros((y_band, x_band))
	saveRASTERfile(output_filename.replace('num', 'all'), ottofile, OUT_DATA)
	'''
	...
	classfile = 'T:\\OUTROS\\TerraClass\\princ_use_of_earth_basin.tif'
	...
	if(os.path.isfile(ottofile)):
		...
		y_band = len(OTTO)
		x_band = len(OTTO[0])
		CLASS = numpy.zeros((5, y_band,x_band))
		n_basin = numpy.amax(OTTO)
	else:
		print('Error to read: %s'%(ottofile))
		sys.exit()
	...
	if(os.path.isfile(classfile)):
		for band in range(1,5+1,1):
			CLASS[band-1] =  readFileBand(classfile, band)
	else:
	...
	if(os.path.isfile(idhm)):
		...
		sys.exit()
	DATA_SAMP = datafile('', 'C:\\Users\\carlos.silveira\\Dropbox\\ANALYSES\\OTTO_2011\\', 'data_msg_ndvi_ndwi_perc_basin_v2011.csv', 'BASIN SAMPLES NDVI NDWI PREC SOLO TRMM_SUM NDWI_SUM IDHM CLASS_P CLASS1 CLASS2 CLASS3 CLASS4', 400, 15)
	...
	for basin in range(1,n_basin+1,1):
		...
		if(sample != 0):
			...
			class_p = numpy.mean(CLASS[0][OTTO == basin])
			class_1 = round(numpy.mean(CLASS[1][OTTO == basin]))
			class_2 = round(numpy.mean(CLASS[2][OTTO == basin]))
			class_3 = round(numpy.mean(CLASS[3][OTTO == basin]))
			class_4 = round(numpy.mean(CLASS[4][OTTO == basin]))
			solo    = round(numpy.mean(SOLO[OTTO == basin]))
			trmm_sum   = round(numpy.mean(TRMM_SUM[OTTO == basin])/100)*100
			ndwi_sum   = numpy.mean(NDWI_SUM[OTTO == basin])
			idhm  = round(numpy.mean(IDHM[OTTO == basin])*10)/10
			DATA_SAMP.addAtrib(basin)
			DATA_SAMP.addAtrib(sample)
			DATA_SAMP.addAtrib(ndvi)
			DATA_SAMP.addAtrib(ndwi)
