...
import sys
import copy
from utils import printlog, sleepTime, delFile, mapDict, extractTarfile, calcMonth, calcDay, calcDoy, readFileBand, incDate
...
class main():
    ...
    if(dif_days  < 31 ):
    	...
    	dif_days = str(dif_days)
    	num_days = int(dif_days[0:2])*31 + int(dif_days[2:4]) + 1
    elif(dif_days < 31131):
        dif_days = str(dif_days)
       	num_days = int(dif_days[0])*366 + int(dif_days[1:3])*31 + int(dif_days[3:5]) + 1
    else:
    	print('--- breakkkkk')
    	sys.exit(1)
