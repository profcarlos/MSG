...
class main():
	...
	for doy in range(1,365,16):
		...
		outfile = outputDir + 'EFIC\\' + str(year) + str('%03d'%doy) + '_EFIC.tif' #'_PAR.tif'
		...
		if(not os.path.isfile(outfile)):
			...
			for i, stat in zip(range(n_stations), numpy.transpose(STATIONS)[0]):
				...
				paser = numpy.logical_and(paser, numpy.logical_and(EFIC>0, EFIC<=1))#(PAR>0, PAR<300))
				if(numpy.sum(paser) == 0):
					...
					index = numpy.mean(EFIC[paser])#(PAR[paser])
