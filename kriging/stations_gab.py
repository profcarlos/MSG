	...
	for doy in range(1,365,16):
		...
		if(not os.path.isfile(outfile)):
			...
			for i, stat in zip(range(n_stations), numpy.transpose(STATIONS)[0]):
				...
				paser = numpy.logical_and(STAT==stat, numpy.logical_and(numpy.logical_and(YEAR==year, MONTH>=month), DAY>=day))
				paser = numpy.logical_and(paser,      numpy.logical_and(numpy.logical_and(YEAR==year, MONTH<=month_up), DAY<=day_up))
				...
				if(numpy.sum(paser) == 0):
					...
					if(not numpy.isnan(index) and not index < 0):
						stat_data = str('%.6f,%.6f,%.2f'%(STATIONS[i][1], STATIONS[i][2], index))
						print(stat_data)
						if(i != n_stations-1):
							stat_data = stat_data + '\n'
